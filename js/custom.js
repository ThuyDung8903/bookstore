jQuery(document).ready(function ($) {
  (function ($) {
    ...
    $(function () {
      ...
      $(document).on("click", function (e) {
        if ($(e.target).is(".category-trigger") === false) {
          $(".category-nav").removeClass("show");
        }
      });
    });
    ...
    $(".product-view-mode a").on("click", function (e) {
      ...
      if (
        shopProductWrap.hasClass("grid") ||
        shopProductWrap.hasClass("grid-four")
      ) {
  ...
  //change input quantity-add-to-cart
  $('#quantity-add-to-cart').on('change', function () {
    var $quantity = $('#quantity-add-to-cart').val();
    var $link = $('#link-add-to-cart').attr('href');
    //bay gio can lam: Neu ô input quantity thay đổi thì link ở Add to cart thay param &quantity=<giá trị input>
    var $new_link = $link.substring(0, $link.indexOf("quantity"));
    $new_link = $new_link + "quantity=" + $quantity;
    //alert($new_link);
    $('#link-add-to-cart').attr('href', $new_link);
  });
  //Page: checkout.php get shipping fee when shipping_method_id select on change
  //initial calculate
  var $element = $('#shipping_method').find('option:selected');
  var $cost = parseFloat($element.attr("cost")).toFixed(2);
  //Your order: Shipping Fee
  $('#shipping_fee').html("$" + $cost);
  var $sub_total = parseFloat($('input[name="sub_total"]').val());
  $('input[name="shipping_fee"]').val($cost);
  var $grand_total = (parseFloat($cost) + parseFloat($sub_total)).toFixed(2);
